...
import { PrintProfileExperimental } from './PrintProfileExperimental';

// ─────────────────────────────────────────────────────────────────────────────
// EXPERIMENTAL PDF root — full copy of PrintProfileApp.tsx.
//
// Rendered in the hidden print window loaded with `?print=exp` (see main.tsx).
// Identical behaviour to the production PrintProfileApp, but renders
// PrintProfileExperimental. See src/lib/featureFlags.ts (PDF_EXPERIMENTAL_ENABLED).
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * Standalone root rendered in the hidden experimental print BrowserWindow
 * (URL `?print=exp`). Bypasses login and app chrome: it fetches the patient
 * directly via the unconditional db IPC, forces the light theme, and renders the
 * experimental print layout.
 */
export const PrintProfileAppExperimental: React.FC<{ patientId: string }> = ({ patientId }) => {
  ...
  if (error) {
    return <div style={{ padding: 24, fontFamily: 'sans-serif', color: '#b91c1c' }}>PDF-Export (experimentell) fehlgeschlagen: {error}</div>;
  }
  if (!data) return null;
