...
import { DIAGNOSE_OPTIONEN, LOKALISATION_CONFIG, serializeLokalisation, restoreLokalisation } from '../lib/diagnose-config';
...
export const EditPatientModal: React.FC<EditPatientModalProps> = ({ isOpen, patient, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (patient && isOpen) {
      ...
      setLokalisationSelections(restoreLokalisation(patient.lokalisation ?? '', stored));
    }
  }, [patient, isOpen]);
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-5 overflow-y-auto">
              ...
              <div className="flex gap-4 pt-2">
                <button type="button" onClick={onClose} className="flex-1 px-6 py-4 bg-slate-100 rounded-2xl text-sm font-black text-slate-600 hover:bg-slate-200 transition-all uppercase tracking-widest">
                  Abbrechen
                </button>
