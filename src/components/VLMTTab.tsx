...
import { prColorCls, PrBadge, AbortBadge, HistoryRowActions, FormSave, HistoryDate, OutOfRangeWarning, confirmNoteOnlySave } from './TestForm';
...
export const VLMTTab: React.FC<VLMTTabProps> = ({ patient, previousResults, onSave, onUpdate, onDelete }) => {
  ...
  const handleSave = async () => {
    // Ein teilweise ausgefüllter Datensatz wird IMMER gespeichert (sonst geht der bis
    // dahin erreichte Lernverlauf verloren). Ein komplett leerer Datensatz nur, wenn
    // eine Notiz vorliegt — dann nach kurzer Rückfrage als normales Ergebnis mit Notiz.
    if (!validate()) return;
    if (!anyValue) {
      if (!note.trim()) return;
